...
const Chap5 = () => {
  ...
  return (
    <Container className="p-4 content">
      ...
      <p>
        <dfn>Deadlock</dfn> is a situation where a set of processes are blocked
        because each process is holding a resource and waiting for another
        resource acquired by some other process.
      </p>

      <p>
        Let’s take an example when two trains are coming toward each other on
        the same track and there is only one track, none of the trains can move
        once they are in front of each other. A similar situation occurs in
      ...
      <ol>
        <li>
          <dfn>If resources have a single instance</dfn> - In this case for
          Deadlock detection, we can run an algorithm to check for the cycle in
          the Resource Allocation Graph. The presence of a cycle in the graph is
          a sufficient condition for deadlock.
        </li>
        <li>
          <dfn>If there are multiple instances of resources</dfn> - Detection of
          the cycle is necessary but not sufficient condition for deadlock
          detection, in this case, the system may or may not be in deadlock
      ...
      <p>
        ...
        is waiting for the resource held by the third process and so on, till
        the last process is waiting for a resource held by the first process.
        This forms a circular chain. For example: Process 1 is allocated
        Resource2 and it is requesting Resource 1. Similarly, Process 2 is
      ...
      <p>
        ...
        checks for the safe state, if after granting request system remains in
        the safe state it allows the request and if there is no safe state it
      ...
      <p>
        ...
        the loan amount from the total money that bank has and if the remaining
      ...
      <ol>
        <li>
          If the request made by the process is less than equal to max need to
          that process.
        </li>
        <li>
      ...
      <p>
        Some data structures that are used to implement the banker's algorithm
        are:
      </p>
      ...
      <p>
        It is an array of length <code>m</code>. It represents the number of
        available resources of each type. If <code>Available[j] = k</code>, then
        there are <code>k</code> instances available, of resource type{" "}
        <code>Rj</code>.
      </p>
      <h6>
        2. <code>Max</code> :
      ...
      <p>
        It is an <code>n x m</code> matrix which represents the maximum number
        of instances of each resource that a process can request. If{" "}
        <code>Max[i][j] = k</code>, then the process <code>Pi</code> can request
        atmost <code>k</code> instances of resource type <code>Rj</code>.
      </p>
      <h6>
        3. <code>Allocation</code> :
      </h6>
      <p>
        It is an <code>n x m</code> matrix which represents the number of
        resources of each type currently allocated to each process. If{" "}
        <code>Allocation[i][j] = k</code>, then process<code>Pi</code> is
        currently allocated <code>k</code> instances of resource type{" "}
        <code>Rj</code>.
      </p>
      <h6>
        4. <code>Need</code> :
      ...
      <p>
        It is a two-dimensional array. It is an <code>n x m</code> matrix which
        indicates the remaining resource needs of each process. If{" "}
        <code>Need[i][j] = k</code>, then process <code>Pi</code> may need
        <code>k</code> more instances of resource type <code>Rj</code> to
        complete its task.
      ...
      <p>
        ...
        information in the graph. That graph is called{" "}
        <dfn>Resource Allocation Graph (RAG)</dfn>.
      </p>
      <p>
