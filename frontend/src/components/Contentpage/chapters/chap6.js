...
const Chap6 = () => {
  ...
  return (
    <Container className="p-4 content">
      ...
      <ul>
        <li> Memory manager is used to keep track of the status of memory locations, whether it is free or allocated. It addresses primary memory by providing abstractions so that software perceives a large memory is allocated to it.</li>
        <li>Memory manager permits computers with a small amount of main memory to execute programs larger than the size or amount of available memory. It does this by moving information back and forth between primary memory and secondary memory by using the concept of swapping.</li>
        <li>The memory manager is responsible for protecting the memory allocated to each process from being corrupted by another process. If this is not ensured, then the system may exhibit unpredictable behavior. </li>
        <li> emory managers should enable sharing of memory space between processes. Thus, two programs can reside at the same memory location although at different times.</li>
      ...
      <ul>
          <li>o	Contiguous memory management schemes</li>
          <li>o	Non-Contiguous memory management schemes</li>
      ...
      <p><b>Fragmentation is of two types −</b></p>
      <ol type='1'>
      <li>External fragmentation-
Total memory space is enough to satisfy a request or to reside a process in it, but it is not contiguous, so it cannot be used.</li>
<li>
...
<p>At the beginning of the process, all the frames remain empty so that all the pages of the processes will get stored in a contiguous way.</p>
...
<p>There are various page replacement algorithms. Each algorithm has a different method by which the pages can be replaced.</p>
<ol type='1'>
  <li><b>	Optimal Page Replacement algorithm →</b> this algorithms replaces the page which will not be referred for so long in future. Although it can not be practically implementable but it can be used as a benchmark. Other algorithms are compared to this in terms of optimality.</li>
  <li><b>	Least recent used (LRU) page replacement algorithm → </b>this algorithm replaces the page which has not been referred for a long time. This algorithm is just opposite to the optimal page replacement algorithm. In this, we look at the past instead of staring at future.</li>
  <li><b>	FIFO → </b>in this algorithm, a queue is maintained. The page which is assigned the frame first will be replaced first. In other words, the page which resides at the rare end of the queue will be replaced on the every page fault</li>
</ol>
